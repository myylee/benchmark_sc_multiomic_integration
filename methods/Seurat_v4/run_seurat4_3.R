...
run_seurat4_fn <- function(in_dir, out_dir,nclust = 7){
    ...
    write_mtx_folder(file.path(out_dir,"seurat4","predicted","ATAC"),unpaired_atac,assay_key="ATAC",slot_key="counts","peak")
    write_mtx_folder(file.path(out_dir,"seurat4","predicted","RNA"),unpaired_atac,assay_key="SCT",slot_key="data","gene")
    
    t2 <- Sys.time()
    ## use '[[1]]' for clean output
    write.table(difftime(t2, t1, units = "secs")[[1]], 
                file = file.path(out_dir,"runtime","seurat4_prediction_time.txt"), 
                sep = "\t",
                row.names = FALSE,
                col.names = FALSE)
