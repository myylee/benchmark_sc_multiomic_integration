...
#     unpaired_atac <- merge(unpaired_atac,paired_atac, add.cell.ids = c("uatac", "patac"))
    ...
    rna <- TransferData(
      ...
      refdata = GetAssayData(unpaired_rna, assay = "RNA", slot = "data"),
      weight.reduction = unpaired_atac[["lsi"]],
      dims = 2:n_lat
    )

    # add predicted values as a new assay
    unpaired_atac[["predicted"]] <- rna
    
    write_mtx_folder(file.path(out_dir,"seurat3","predicted/ATAC/"),unpaired_atac,assay_key="ATAC",slot_key="counts","peak")
    write_mtx_folder(file.path(out_dir,"seurat3","predicted/RNA/"),unpaired_atac,assay_key="predicted",slot_key="data","gene")
    
    t2 <- Sys.time()
    ## use '[[1]]' for clean output
    write.table(difftime(t2, t1, units = "secs")[[1]], 
                file = file.path(out_dir,"runtime","seurat3_prediction_time.txt"), 
                sep = "\t",
                row.names = FALSE,
                col.names = FALSE)
