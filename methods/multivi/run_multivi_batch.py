...
def run_multivi_fn(in_dir,out_dir):
    ...
    scvi.settings.seed = 420
    adata_prna = read_mtx_folder(in_dir+"/paired_RNA/",
                                       "Gene Expression",
                                       ["gene"],
                                       ["barcodes","batch"])

    adata_patac = read_mtx_folder(in_dir+"/paired_ATAC/",
                                       "Peaks",
                                       ["peak"],
                                       ["barcodes","batch"])

    adata_urna = read_mtx_folder(in_dir+"/unpaired_RNA/",
                                       "Gene Expression",
                                       ["gene"],
                                       ["barcodes","batch"])

    adata_uatac = read_mtx_folder(in_dir+"/unpaired_ATAC/",
                                       "Peaks",
                                       ["peak"],
                                       ["barcodes","batch"])
    ...
    os.makedirs(out_dir+"/multivi", exist_ok=True)
    res_df.to_csv(out_dir+"/multivi/multivi_result.csv")
    mvi.save(out_dir+"/multivi/trained_multivi", overwrite=True)
    stop = timeit.default_timer()
    print('Time(s): ', stop - start)  
    os.makedirs(out_dir+"/runtime", exist_ok=True)
    # record time 
    print(stop - start,  file=open(out_dir+'/runtime/multivi_runtime.txt', 'w'))
    print("------ Done ------")
    print("------ Prediction ------")
    start = timeit.default_timer()
    ...
    write_adata(rna_pred_save, out_dir+"/multivi/predicted/RNA/",
                           "RNA","gene",bc="barcodes",feature_name='feature',transpose=True)
    write_adata(adata_atac_save, out_dir+"/multivi/predicted/ATAC/",
                           "ATAC","peak",bc="barcodes",feature_name='feature',transpose=True)

    stop = timeit.default_timer()
    print('Time(s): ', stop - start)  
    # save prediction time 
    print(stop - start,  file=open(out_dir+'/runtime/multivi_prediction_time.txt', 'w'))
